...
- **Insulin Estimator** — An interactive tool (not a chart) that suggests an insulin dose. Enter current BG, insulin type, target BG, time-to-target, max dose, an estimated drop-per-unit, and an optional overnight drift; it recommends an integer dose and plots the projected BG with vs. without insulin against a target line. Uses a replaceable pharmacokinetic estimator (`InsulinEstimator`). **Estimation aid only — not medical advice.** (Empirical auto-fitting of the per-unit sensitivity and drift from your own history is planned.)
